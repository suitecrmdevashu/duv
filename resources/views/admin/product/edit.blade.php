...
@section('content')
  <div class="card">
    <div class="card-body row">
      <div class="col-md-6 col-12">
        <form action="{{ route('productEditSave', ['product' => $product->title, 'id' => $product->id]) }}" method="post" enctype="multipart/form-data">
          ...
          <div class="form-group">
              ...
              <textarea name="desc" id="desc" cols="30" class="form-control autosize @error('desc') is-invalid @enderror" placeholder="Enter Description">{{ $product->desc }}</textarea>
              @error('desc')
      ...
      <div class="col-md-6 col-12">
        <form method="post" action="{{ route('productAddImagesSave') }}" enctype="multipart/form-data" class="dropzone mt-4" id="dropzone" style="border-radius: 12px; min-height: 150px; border: 2px solid rgba(0,0,0,0.3); background: white; padding: 20px 20px;">
            <input type="hidden" name="id_product" value="{{ $product->id }}">
            <div class="dz-message" data-dz-message>
                <span>Upload Gallery Images</span><br>
                <svg width="42" height="42" viewBox="0 0 42 42" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M22.75 12.25H19.25V19.25H12.25V22.75H19.25V29.75H22.75V22.75H29.75V19.25H22.75V12.25ZM21 3.5C11.34 3.5 3.5 11.34 3.5 21C3.5 30.66 11.34 38.5 21 38.5C30.66 38.5 38.5 30.66 38.5 21C38.5 11.34 30.66 3.5 21 3.5ZM21 35C13.2825 35 7 28.7175 7 21C7 13.2825 13.2825 7 21 7C28.7175 7 35 13.2825 35 21C35 28.7175 28.7175 35 21 35Z" fill="black" fill-opacity="0.3"/>
                </svg>
            </div>
            @csrf
        </form>
  ...
  $.ajax({
    ...
    success: function(response) {
      ...
      $.each(response, function(key, value) {
        data += '<div class="col-lg-3 col-md-3 col-12">'
        data += '<div class="product-image-item mb-4">'
        data += '<button class="btn btn-danger btn-sm delete-image-product" data-id="'+value.id+'" style="position:absolute;z-index:9;right:0;" onClick=alertconfirm("'+value.path+'")><i class="bi bi-trash"></i></button>'
        data += '<img src="{{ asset("shop/products/")}}/'+value.path+'" style="width: 100%; height: 100px;">'
        data += '</div>'
        data += '</div>'
      })
